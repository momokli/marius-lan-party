# 🎒 Gear-Checkliste

> BYO = bring your own (jeder bringt sein Zeug selbst).
> Gemeinschafts-Gear wird Do aufgebaut (Momo + Matheo).
> **Strategie: ausleihen statt kaufen** — wer hat was schon? (Momo hat im Lab bestimmt Switch + Kabel)

## Leihen statt kaufen 📦
- [ ] **Switch** (Gbit, 8 Ports) — Momo Lab? oder jemand anders
- [ ] **LAN-Kabel** (6–8 Stück, 3/5/10m) — Momo Lab?
- [ ] **Mehrfachsteckdosen** (3–4) — wer hat welche übrig?
- [ ] **Kabeltrommel/Verlängerung** — falls nötig
- [ ] **Ventilator** — von zuhause mitbringen
- [ ] **Bluetooth-Box** — wer hat eine?
- [ ] **HDMI/DP-Kabel + Adapter** — Ersatz von wem?

## Schlafen 😴 (vor Ort „nur shit“)

**3 Schlafplätze nötig** — Optionen (Geizhals-Baseline, Stand 2026-08):

| Option | Preis ab | Händler | Link |
|--------|----------|---------|------|
| Mil-Tec Alu Feldbett | € 47,99 | Geizhals-Vergleich | [Link](https://geizhals.de/mil-tec-feldbett-14402001-a2936059.html) |
| Camping Feldbett olive | € 55,00 | Geizhals-Vergleich | [Link](https://geizhals.de/camping-feldbett-olive-a3892592.html) |
| Outwell Posadas Single | € 61,95 | Geizhals-Vergleich | [Link](https://geizhals.de/outwell-posadas-single-campingliege-470304-a1515368.html) |
| Coleman Packaway | € 70,30 | Geizhals-Vergleich | [Link](https://geizhals.de/coleman-packaway-campingliege-2176135-a3230138.html) |
| Outwell Posadas XL | € 77,90 | Geizhals-Vergleich | [Link](https://geizhals.de/outwell-posadas-xl-campingliege-470305-a1832308.html) |
| KingCamp Ultralight XL | € 80,93 | Geizhals-Vergleich | [Link](https://geizhals.de/kingcamp-klick-feldbett-xl-v166985.html) |
| Decathlon Bed Air (aufblasbar, Lattenrost 70x200) | ~70–90 € (Refurbished verfügbar) | Decathlon | [Link](https://www.decathlon.de/p/campingbett-aufblasbar-mit-lattenrost-bed-air-70x200-cm-blau/309887/c70c98m8595949) |

- [ ] **3 Schlafplätze klären**: kaufen (2× Mil-Tec/Decathlon reicht?) oder Kleinanzeigen gebraucht
- [ ] Schlafsack/Decke + Kissen je Person (BYO)
- [ ] Luftmatratze-Pumpe (falls aufblasbar)

## Jeder bringt mit (BYO)
- [ ] PC/Laptop + Netzteil
- [ ] Monitor + Kabel (HDMI/DP) + ggf. USB-C-Adapter
- [ ] Tastatur, Maus, Mauspad
- [ ] Kopfhörer/Headset
- [ ] Controller (falls gewünscht)
- [ ] Steam-Accounts vorher einloggen, Updates vorher ziehen! (kein Download-Roulette bei der Party)

## Strom ⚡ (wichtigster Flaschenhals!)
- [ ] Mehrfachsteckdosen mit Überspannungsschutz (3–4 Stück)
- [ ] Kabeltrommel/Verlängerungskabel (falls Steckdosen knapp)
- [ ] Power-Strips pro Person (je 1–2)

## Netzwerk 🌐
- [ ] Gbit-Switch (8 Ports reicht locker für 4)
- [ ] LAN-Kabel: 6–8 Stück in verschiedenen Längen (3m, 5m, 10m)
- [ ] Kabeltester (falls was nicht geht)
- [ ] WLAN-Router als Fallback (falls Kabel nicht bis zum Sofa reichen)

## Kabel & Adapter 🔌
- [ ] HDMI-Kabel (2–3 Ersatz)
- [ ] DisplayPort-Kabel (1–2)
- [ ] USB-C-auf-HDMI-Adapter (für Laptops/MacBooks)
- [ ] USB-Verlängerungen (für Maus/Empfänger)
- [ ] Klett-Kabelbinder / Kabelmanagement (sonst Dschungel)

## Audio 🎧
- [ ] Jeder Headset (BYO)
- [ ] Kleine Bluetooth-Box (Pausenmusik)
- [ ] AUX-Kabel (falls Box kein BT)

## Komfort 🛋️
- [ ] Ventilator (4 PCs = Heizung!)
- [ ] Kleine Lampe/LED (Dunkel fürs Zocken, aber nicht Stolperfalle)
- [ ] Decke/Kissen (wer auf dem Sofa pennen will)
- [ ] Müllbeutel in Reichweite

## Optional
- [ ] Webcam (falls LanHub-Livestream gewünscht)
- [ ] Zweitmonitor für Streaming/Charts
- [ ] Steam Deck (fürs Bett zwischendurch)
- [ ] Aufstellwürfel/Notizzettel für Scoreboards (Turnier-Modus!)

## Check vor der Party
- [ ] Steam: alle Spiele installiert + geupdated
- [ ] Server-Tests: Factorio/MC/CS2-Server laufen (siehe `LAN-SERVER.md`)
- [ ] LanHub: Event + Musicbot ready
